...
(function() {
  $(document).ready(function() {
    return $("p.control_group").focusout(function() {
      if (!$("p.control_group").hasClass("error")) {
...
ClientSideValidations.formBuilders['SimpleForm::FormBuilder'] = {
  add: function(element, settings, message) {
    ...
    settings.wrapper_tag = ".control_group";
...
jQuery(function($) {
  $(document.body).on('click', 'form a.add_nested_fields', function() {
    // Setup
  ...
  $(document.body).on('click', 'form a.remove_nested_fields', function() {
    ...
    common: {
      init: function() {
        $('.lang_menu a').click(function() {
          if (document.location.toString().indexOf('en/index.html') !== -1) {
        ...
        return $('.hide-btn').click(function() {});
      }
    }
  };
...
(function() {
  MBLT.background = {
    blocker: false,
    upd: Math.round(new Date().getTime() / 1000),
    height: 0,
    init: function() {
      setInterval(MBLT.background.update, 3500);
      return console.log($(".background.container").height());
    },
    update: function() {
      var chain, shift, slideDown;

      chain = function(f, args_array) {
        var elem;

        console.log('left: ' + args_array.length);
        elem = args_array.pop();
        if (elem) {
          return f.call(null, elem, function() {
            return chain(f, args_array);
          });
        } else {
          return console.log('finish!');
        }
      };
      slideDown = function(elem, container, callback) {
        $(".background.container").css({
          'max-height': $(".background.container").height()
        });
        ...
        return elem.css({
          ...
          container.find('img:last-child').remove();
          return callback();
        });
      };
      shift = function(src, callback) {
        var last_left, new_elem;

        last_left = $(".left-images img:last-child");
        slideDown(last_left.clone(), $(".right-images"), function() {});
        new_elem = $("<img src='" + src + "'/>");
        return slideDown(new_elem, $(".left-images"), callback);
      };
      if (!MBLT.background.blocker) {
        console.log('update fiued!');
        MBLT.background.blocker = true;
        return $.getJSON('/background/upd?upd=' + MBLT.background.upd, function(obj) {
          console.log("answer got, photos: " + obj.images.length);
          $({}).imageLoader({
            images: obj.images,
            async: false,
            allcomplete: function() {
              console.log('compl fired!');
              chain(shift, obj.images);
              return MBLT.background.blocker = false;
            }
          });
          if (!obj.images.length) {
            MBLT.background.blocker = false;
          }
          return MBLT.background.upd = obj.upd;
        });
      } else {
...
MBLT.home = {
    index: function() {
        $(function() {
            ...
            // var timer = setInterval("slider_next();", 9000);
            // $(".next").click(function() {
            //     clearInterval(timer);
            //     slider_next()
            // });
        ...
        function slider_next() {
            if(performing) {
                ...
                next = $(".banner.active").next(".banner")
                if(next.length == 0) {
                    next = $(".banner-wrapper").children(".banner:first-child")
                }
        ...
        function slider_prev() {
            if(performing) {
                ...
                elem = $(".banner.active")
                elem.css('z-index', 100)
                next = $(".banner.active").prev(".banner")
                if(next.length == 0) {
                    next = $(".banner-wrapper").children(".banner").last()
                }
                // next.addClass("active")
                next.fadeIn('slow')
                // next.css('display', 'block')
  ...
  MBLT.speakers = {
    index: function() {
      $(window).hashchange(function() {
        $('.js-speakers-list-wrapper li.active').removeClass('active');
        if (document.location.hash) {
          return $(document.location.hash).addClass('active');
        }
      });
...
(function() {
  MBLT.timetable = {
    index: function() {
      var room_link_selector;

      room_link_selector = '.hall-menu a';
      $(window).hashchange(function() {
        var $room_div, $this, current_room;

        current_room = document.location.hash || $("" + room_link_selector + ":first").attr('href');
        $this = $("" + room_link_selector + "[href=" + current_room + "]");
        $room_div = $($this.attr('href') + '_wrapper');
        if ($this.is('.active')) {
          return;
        }
        $(room_link_selector).removeClass('active');
        $this.addClass('active');
        if ($room_div.is(':visible')) {
          return $room_div.siblings('.js_room_wrapper').hide();
        } else {
          return $room_div.siblings('.js_room_wrapper').fadeOut('fast', function() {
            return $room_div.fadeIn('fast');
          });
        }
      });
      return $(window).hashchange();
...
MBLT.UTIL = {
    ...
    init: function() {
        ...
        MBLT.UTIL.exec( "common" );
        MBLT.UTIL.exec( controller );
        MBLT.UTIL.exec( controller, action );
    }
};
